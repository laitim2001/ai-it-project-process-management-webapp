...
> **v2.2 更新** (2025-11-26):
> - **重大更新**: `startup.sh` 現在自動執行 Seed，部署後不再需要手動 Seed
> - 更新 Seed Data 章節說明自動化機制
> - 新增 startup.sh 執行流程說明
>
> **v2.1 更新** (2025-11-25):
...
> ✅ **v2.2 重要更新**: 從 v1.3.0 開始，`docker/startup.sh` 會在容器啟動時**自動執行** Migration 和 Seed。
> 部署後不再需要手動執行 Seed，但仍可使用 `/api/admin/seed` API 進行驗證或緊急修復。

#### 2.6.0 自動化機制 (v1.3.0+)

容器啟動時，`startup.sh` 會自動執行以下步驟：
...
⚠️ **注意**: 現在由 startup.sh 自動執行。只在特殊情況下手動執行
...
#### 2.6.2 Seed Data (現在自動執行)

> ✅ **v1.3.0+**: Seed 現在由 `startup.sh` 自動執行，正常情況下**不需要手動執行**。

**為什麼 Seed 很重要?**
- User 表的 `roleId` 字段引用 Role 表，如果 Role 表為空，用戶註冊會失敗 (P2003 外鍵約束錯誤)
- BudgetPool 需要 Currency 表資料
- 自動 Seed 使用 `upsert`，確保冪等性（重複執行不會出錯）

**備用方案: 如需手動執行 Seed**:

如果自動 Seed 失敗或需要驗證，可使用以下方式：
...
# 驗證 Seed 狀態
curl "https://<APP_NAME>.azurewebsites.net/api/admin/seed" \
  -H "Authorization: Bearer <NEXTAUTH_SECRET>"
```
