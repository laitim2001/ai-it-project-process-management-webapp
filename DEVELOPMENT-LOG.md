...
**背景**:
- 專案需要統一的代碼規範，讓 AI 助手在不同目錄工作時能遵循一致的標準
- 原有子目錄 CLAUDE.md 文件內容較簡短，需要豐富化並與規則文件建立連結

**實現內容**:

1. **創建 `.claude/rules/` 規則框架** (10 個文件)
   - `index.md` - 規則索引和使用指南
   ...
   - `i18n.md` - 國際化規範（含重複 Key 問題解決）
   - `typescript.md` - TypeScript 約定
   - `scripts.md` - 開發腳本規範
   - `documentation.md` - 文檔規範

2. **更新所有子目錄 CLAUDE.md** (7 個文件)
   - `apps/web/src/components/CLAUDE.md` (685 行) → 引用 `components.md`
   - `apps/web/src/components/ui/CLAUDE.md` (592 行) → 引用 `ui-design-system.md`
   - `packages/api/src/routers/CLAUDE.md` (479 行) → 引用 `backend-api.md`
   - `packages/api/src/lib/CLAUDE.md` (534 行) → 引用 `backend-api.md`
   - `packages/db/prisma/CLAUDE.md` (827 行) → 引用 `database.md`
   - `scripts/CLAUDE.md` (622 行) → 引用 `scripts.md`
   - `claudedocs/CLAUDE.md` (497 行) → 引用 `documentation.md`

3. **Bug 修復** (FIX-095 ~ FIX-097)
   - FIX-095: Middleware 路由保護邏輯增強
   - FIX-096: Layout Hydration Error 修復
   - FIX-097: href 模板字面量語法修正

**技術決策**:
- 使用 YAML frontmatter 的 `applies_to` 欄位定義規則適用路徑
- 每個規則文件專注一個領域（關注點分離）
- CLAUDE.md 與 rules 建立雙向連結，方便 AI 助手導航
...
### 2025-12-15 | 🔧 FIX-008 + FIX-009: 專案狀態回退與表格顯示修復 | 完成 ✅

**類型**: 修復 | **負責人**: AI 助手 | **狀態**: ✅ 完成

**FIX-008: 專案狀態卡在 InProgress 無法回退修復**

**問題**: 刪除預算提案後，專案狀態會卡在 InProgress，無法退回 Draft 或刪除專案

**根因分析**:
- 當預算提案被批准時，專案狀態自動改為 `InProgress`
- 但當提案被回退 (revertToDraft) 或刪除時，專案狀態未同步回退
- 導致專案狀態與實際提案狀態不一致

**修復內容**:
1. **自動回退機制**: `budgetProposal.revertToDraft` 新增檢查邏輯
   - 當從 Approved 狀態回退時，檢查是否還有其他已批准的提案
   - 若無，自動將專案狀態回退到 Draft
2. **手動回退 API**: 新增 `project.revertToDraft`
   - 允許 Manager/Supervisor/Admin 手動回退專案狀態
   - 檢查是否有已批准的提案存在
   - 必填回退原因
3. **前端 UI**: 專案詳情頁新增「退回草稿」按鈕
   - AlertDialog 確認對話框
   - 原因輸入欄位

**修改文件** (4 個):
- `packages/api/src/routers/budgetProposal.ts` (915-934 行)
- `packages/api/src/routers/project.ts` (2103-2217 行)
...
**根因**: 使用 `truncate` CSS class 截斷文字，且 Project Name 欄位寬度只有 18%

**修復內容**:
1. 調整欄位寬度百分比 (Project Name: 18% → 25%)
2. 移除 `truncate` CSS class，改用 `break-words` 允許文字換行
3. Description 使用 `line-clamp-2` 限制最多 2 行

**修改文件** (1 個):
- `apps/web/src/components/project-summary/ProjectSummaryTable.tsx` (333, 353-362 行)
...
1. **唯一真相來源**: `packages/api/src/lib/schemaDefinition.ts`
   - 定義所有 31 個 Prisma 模型的預期欄位
   - 定義欄位類型對照表 (用於生成 ALTER TABLE)

2. **新增 API**:
   - `health.fullSchemaCompare` (GET): 完整對比所有 31 個表格與實際數據庫
