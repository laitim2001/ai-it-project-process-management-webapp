...
- [ ] 在某專案下新增 ProjectExpense 費用表（表頭，設年度），再於表下新增明細，設定幣別/費用類別/OpCo
- [ ] 該明細 12 月網格可分別填「月度預算」與「月度實際」並儲存
- [ ] 明細年度預算/實際 = 對應 12 月加總（自動）
- [ ] 金額顯示為 USD 主值 +（有匯率時）換算次值
- [ ] 專案詳情頁可見此模組，且與既有 PO→Expense 預算使用卡片**清楚分開、無相加**
- [ ] 明細可編輯/刪除/排序
- [ ] `pnpm db:migrate`/`db:generate`/`typecheck`/`lint`/`validate:i18n` 通過
