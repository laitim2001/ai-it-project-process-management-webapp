# CHANGE-049: OM 明細以選定幣別輸入、存檔換算為 USD + 顯示固定 USD/HKD

> **建立日期**: 2026-06-03
> **完成日期**: 2026-06-03
> **狀態**: ✅ 已完成（純前端；待手測）
> **優先級**: High（修正 CHANGE-042 遺留的金額語意矛盾）
> **類型**: 現有功能增強 + Bug 修復（延續 CHANGE-042 / CHANGE-048）
> **前置依賴**: CHANGE-042（`lib/currency.ts`、`DualCurrency`）、CHANGE-048（月度 HKD、`hkdRate` 取得）；Admin 已於 `/settings/currencies` 填 HKD `exchangeRate`
...
> **🔢 編號變更（2026-06-09）**: 本變更原號為 **CHANGE-045**（2026-06-03）。所屬 OM 雙幣批次（044/045/046）從未合併進 main 且擱置於 git stash；因 main 於 2026-06-09 另用 CHANGE-044，本批統一重配：**044→048、045→049、046→050**。原始碼存於分支 `wip/om-dual-currency-044-046`。
...
| **D3 換算位置** | 放前端表單（輸入正規化）：不改 API 合約、不需 migration；與 CHANGE-048 前端換算一致；載入方向本就需在表單做，故save/load 對稱 |
...
- **未受影響**: Schema、API、OM Summary（仍讀 USD）、CHANGE-048 月度欄
...
### 7.3 注意事項
- 🟡 **dev server**：本變更純前端，hot-reload 即生效，不需重啟（但 CHANGE-048 的月度 HKD 存檔仍需重啟以載入新 Prisma Client）。
- 🟡 **換算位置在前端**：屬輸入正規化（與 CHANGE-048 一致、載入方向本就需在表單做）；業務彙總邏輯仍在 API/USD。若日後要嚴格遵守「業務邏輯在 API」，可改由 `addItem`/`updateItem` 內換算（須同步調整載入方向）。
- 🟡 **編號**：本變更原配 CHANGE-045（緊接 CHANGE-044 OM 月度 HKD），2026-06-09 因撞號重配為 **CHANGE-049**（詳見頂部編號變更註記）。

---

## 8. 相關文檔

- 延續：`CHANGE-042-om-expense-dual-currency-display.md`（雙幣別顯示、`lib/currency.ts`）、`CHANGE-048-om-monthly-dual-currency-input.md`（月度 HKD、`hkdRate`）
