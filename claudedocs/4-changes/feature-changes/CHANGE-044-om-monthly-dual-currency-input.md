# CHANGE-044: OM Monthly 可輸入並持久化的 HKD 金額欄（USD→HKD 自動換算）+ 修月度卡片重疊
...
`OMExpenseMonthly` 新增：
```prisma
actualAmountHKD Float?  // CHANGE-044: HKD 實際支出（可 null；既有資料以換算值帶入顯示，存檔後寫死）
```
- **可為 null** 的理由：既有 row 無此值，避免脆弱的 SQL backfill；改由前端載入時以換算值帶入。
- Migration：`20260603032209_change044_om_monthly_hkd` —
...
- `packages/db/prisma/migrations/20260603032209_change044_om_monthly_hkd/`
...
- 🟡 **編號修正**：本變更原誤用 `CHANGE-043`（已被 `CHANGE-043-budget-proposal-meeting-minutes-fields` 占用），全面改為 **CHANGE-044**，含 migration 名稱、6 個原始碼檔的註解。migration 資料夾以 rename + 同步 `_prisma_migrations.migration_name` 處理（`migration.sql` 內容不變、checksum 有效），非 drop/recreate。
