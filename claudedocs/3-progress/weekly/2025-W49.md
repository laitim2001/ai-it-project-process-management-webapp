...
- ✅ **本地 UI Bug 修復** (12/4)
  - 問題：Proposal 詳情頁的專案連結跳轉錯誤 (`/projects/$%7Bproposal.project.id%7D`)
  - 根本原因：使用普通引號而非反引號，導致模板字符串未解析
  - 修復位置：`proposals/[id]/page.tsx` 第 226 行和第 333 行
  - 修復內容：`href="/projects/${...}"` → `href={`/projects/${...}`}`

- ✅ **Expense 創建 budgetCategoryId 外鍵錯誤修復** (12/4) - **FIX-006**
  - 問題：創建 Expense 時報錯 `Foreign key constraint violated: Expense_budgetCategoryId_fkey`
  - 根本原因：繼承的 budgetCategoryId 指向不存在的 BudgetCategory 記錄
  - 解決方案：在 `expense.ts` 創建前驗證 budgetCategoryId 是否有效
  - 部署版本：v20-fix-expense-category

- ✅ **系統化 Schema 診斷和修復工具** (12/4) - **FIX-007** - **重大功能**
  - 問題：本地環境正常但 Azure 環境出錯（Schema 不同步）
  - 根本原因：本地使用 `db push` 同步，Azure 只執行 migrations
  - 新增診斷 API：
    - `health.schemaCompare` - 比較 schema.prisma 定義 vs 資料庫實際欄位
  - 新增修復 API：
    - `health.fixExpenseItemSchema` - 修復 ExpenseItem 缺失欄位
    - `health.fixAllSchemaIssues` - **一鍵修復所有 Schema 不同步問題**
  - 修復的 Azure 資料庫欄位：
...
- ✅ **部署文檔更新** (12/4)
  - 更新 SITUATION-7-AZURE-DEPLOY-COMPANY.md
  - 新增 `schemaCompare`、`fixExpenseItemSchema`、`fixAllSchemaIssues` 診斷工具說明

- ✅ **FEAT-006: Project Summary Tab 規劃完成** (12/5)
  - 完成需求分析和設計稿審查
  - 識別需要新增的 8 個 Project 欄位 + 1 個中間表
  - 創建功能規劃目錄 (FEAT-006-project-summary-tab/)
...
- ✅ **FEAT-007: OM Expense 表頭-明細架構重構** (12/5) - **重大功能重構**
  - **Phase 0: 規劃準備** - 完成架構分析、識別 25 個受影響檔案
  - **Phase 1: Schema 設計與遷移** - 新增 OMExpenseItem 模型
    - OMExpense → OMExpenseItem → OMExpenseMonthly 三層架構
    - 支援多明細項目、每項目獨立 OpCo 和幣別
    - 向後兼容設計（舊欄位標記 @deprecated）
  - **Phase 2: API Router 重構** - 新增 6 個 procedures
    - createWithItems, addItem, updateItem, removeItem
    - reorderItems, updateItemMonthlyRecords
  - **Phase 3-4: 前端組件與頁面改造**
    - 新增 OMExpenseItemForm, OMExpenseItemList, OMExpenseItemMonthlyGrid
    - 整合 @dnd-kit 拖曳排序功能
  - **Phase 5: I18N 與品質檢查** - 2066 個翻譯鍵，驗證通過
  - **Phase 6: 數據遷移與測試** - 完成功能測試和 Bug 修復
    - 修復 I18N itemsSection/vendor.noSelection 問題
    - 修復 TypeScript OperatingCompany._count 類型錯誤
  - **Phase 7: 部署與驗收** - 部分完成
    - Git 提交推送完成（7 個 commits）
    - CLAUDE.md 文檔已更新
    - Azure 部署待執行
  - 文檔位置: `claudedocs/1-planning/features/FEAT-007-om-expense-header-detail-refactoring/`

- ✅ **FEAT-007 Bug Fixes** (12/5 下午) - **4 個 UI/UX Bug 修復**
  - **Bug 1**: Select 空字串錯誤 - Radix UI Select 不允許 `value=""`
    - 修復：Currency Select 改用 `value="__none__"` 作為佔位值
    - 位置：`OMExpenseItemForm.tsx` 第 372-386 行
  - **Bug 2**: 月度記錄 Tab 無法訪問（灰色禁用狀態）
    - 修復：移除 `disabled={!selectedItemId}` 限制
    - 新增項目選擇器下拉選單供用戶選擇明細項目
    - Tabs 改為受控組件（controlled component）
  ...
  - **Bug 4**: 日期格式不符錯誤
    ...
    - 新增 `formatDateForInput()` 輔助函數
    - 位置：`OMExpenseItemForm.tsx` 第 101-114 行
  - Git Commits：`58fbd99`, `def439a`, `09bb59f`, `40a113a`

- ✅ **CHANGE-004: OM Summary 表頭-明細階層顯示** (12/7) - **功能完成**
  - **Phase 1: API 更新**
    - `omExpense.getSummary` 新增 `headers` 資料結構
    - 支援表頭層級的預算和實際金額彙總
    - OpCo 過濾同時支援舊架構（表頭 opCoId）和新架構（明細 items.opCoId）
  - **Phase 2: 前端組件**
    - 新增 `OMExpenseHeaderSection` 組件顯示表頭資訊
    - 整合到 `OMSummaryDetailGrid.tsx`
    - 新增 FileText icon 圖示
  - **Phase 3: I18N 翻譯**
    - 新增 `headerTotal`、`headerSubTotal` 翻譯鍵
  - **Bug Fixes** (6 項):
    1. OpCo 過濾應過濾明細項目而非表頭 - 使用 OR 條件
    2. 移除重複的 Subtotal 行 - 保留單一 "Sub Total"
    3. Project Summary 表格欄位不對齊 - 使用 table-fixed + colgroup
    4. React hydration 錯誤 - 移除 colgroup 內空白
    5. Budget Pool 編輯頁 Currency 顯示空值 - 傳遞 currencyId
    6. Quote Amount (TWD) → Quote Amount 翻譯修改
  - 文檔位置: `claudedocs/4-changes/feature-changes/CHANGE-004-om-summary-header-detail-display.md`

- ✅ **索引維護機制優化** (12/8) - **重大改進**
  - **check-index-sync.js 腳本更新** (v1.1.0 → v1.2.0)
    - 新增 `isClaudedocsGranularFile()` 函數排除 150+ 細粒度文檔
    - 新增 Windows 路徑兼容性修復（反斜線 → 正斜線轉換）
    - 新增 barrel export (index.ts) 排除邏輯
    - 新增子目錄 CLAUDE.md 排除邏輯
  - **PROJECT-INDEX.md 更新**
    - 新增 15+ 重要文件索引
    - 包含：auth.config.ts, playwright.config.ts, [locale] 布局, API 路由
    - 包含：Azure 部署文檔, Quotes 頁面, 環境檢查腳本
    - 文件統計：360+ → 380+ 已索引文件
  - **結果**：索引建議從 159 個減少到 0 個 (100% 修復)

### 進行中
- ⏳ **FEAT-006: Project Summary Tab** - Phase 0 完成，等待開始 Phase 1
...
### 挑戰 11: Radix UI Select 空字串限制 (12/5)
- **問題**: `<SelectItem value="">` 會導致 Radix UI Select 報錯
- **原因**: Radix UI Select 不允許空字串作為 value prop
- **解決方案**: 使用佔位值 `"__none__"` 並在 onValueChange 中轉換回空字串
- **學習**: Radix UI 組件有特定限制，需注意官方 API 規範

### 挑戰 12: 日期格式轉換問題 (12/5)
- **問題**: HTML input[type="date"] 報錯 "does not conform to the required format, 'yyyy-MM-dd'"
- **原因**: API 返回 ISO 8601 格式 (2025-12-08T00:00:00.000Z)，但 input 需要 yyyy-MM-dd
- **解決方案**: 新增 `formatDateForInput()` 輔助函數處理格式轉換
- **學習**: 表單日期欄位需要統一的格式轉換層
...
## Git 提交記錄
```
# FEAT-007 Bug Fixes (12/5 下午)
40a113a fix(om-expense): 修復日期格式轉換問題 - ISO to yyyy-MM-dd
09bb59f fix(om-expense): 移除不存在的 i18n key common.currency.twd
def439a fix(om-expense): 修復月度記錄 Tab 禁用問題和缺失 i18n 鍵
58fbd99 fix(om-expense): 修復 Select 空字串錯誤和 Tab 控制問題

# FEAT-007 系列提交 (12/5 上午)
9c03d92 docs(FEAT-007): 更新 CLAUDE.md 反映表頭-明細架構重構
ad4916f fix(FEAT-007): Phase 6 完成 - Bug 修復與測試驗證
6e2c868 feat(FEAT-007): Phase 5 完成 - I18N 與品質檢查
4abe3a9 feat(FEAT-007): Phase 3-4 完成 - 前端組件與頁面改造
d04dd64 docs: 更新 FEAT-007 Phase 2 進度文檔
c779fca feat(api): FEAT-007 Phase 2 - OM Expense API Router 重構完成
503b368 feat(schema): FEAT-007 Phase 1 - OM Expense 表頭-明細架構 Schema 重構
