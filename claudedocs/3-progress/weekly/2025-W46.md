...
### 技術債務
- ~~@itpm/auth 的 11 個 TypeScript errors~~（已在 2025-11-14 修復）
- ~~@itpm/api 的 21 個 TypeScript errors~~（已在 2025-11-14 修復）
- ~~E2E 測試的 60+ 個 TypeScript errors~~（已在 2025-11-14 修復）
...
## ✅ 2025-11-14 更新 1: JSDoc 後續改進與 TypeScript 錯誤修正
...
- **添加**: 26 個 @related 引用，建立完整的依賴關係文檔
...
- **添加**: 33 個 @related 引用，涵蓋 API Router、資料模型、組件

### 2. 組件路徑修正 (2個組件) ✅
- `CommentSection.tsx`: 相對路徑 → 絕對路徑 (3個修正)
- `notification/index.ts`: 添加 @features 和 @dependencies 標籤

### 3. TypeScript 類型錯誤修正 (92+ 個錯誤)

#### packages/auth (11個 → 0個) ✅
- NextAuthOptions 類型處理
- Prisma Adapter 導入路徑
- User 介面可選屬性
- JWT callback undefined 處理

#### packages/api (21個 → 0個) ✅
- notification.ts: ctx.db → ctx.prisma (12處)
- dashboard.ts: 類型訪問修正 (7處)
- quote.ts: 關聯查詢修正 (1處)
- trpc.ts: 類型擴展導入 (1處)

#### apps/web/e2e (60+ 個 → 0個) ✅
- 類型導入錯誤: `import { Page }` → `import { type Page }`
- 可選屬性錯誤: 使用可選鏈和 nullish coalescing
- Page.evaluate 泛型錯誤: 正確的泛型參數
- String split 錯誤: 添加默認值

### 📊 改進成果

**代碼品質**:
- JSDoc 覆蓋率: 156/156 (100%)
- TypeScript 錯誤: 92+ → 0 (核心套件 + E2E 測試)
- @related 路徑警告: 62 → ~20 (70%+ 改善)
- @related 引用: +59 個新引用

**文件變更**:
- 修改文件: ~30 個
- Git Commits: 2 個 (後續修正)

**技術債務清除**:
- ✅ packages/auth TypeScript 錯誤
- ✅ packages/api TypeScript 錯誤
- ✅ E2E 測試 TypeScript 錯誤
- ✅ JSDoc @related 路徑完善

---

## ✅ 2025-11-14 更新 2: 創建 .claude.md 模式文檔系統

### 1. 項目背景
**目的**: 為所有關鍵目錄創建 `.claude.md` 模式文檔，記錄該模組使用的特定模式和約定，供 AI 助手和開發者參考
...
#### 核心架構層 (6 個目錄)
1. **packages/api/src/routers/** - tRPC API Router 開發模式
2. **packages/db/prisma/** - Prisma Schema 和資料遷移模式
3. **packages/auth/src/** - NextAuth.js 認證和授權模式
4. **apps/web/src/app/[locale]/** - Next.js App Router 頁面路由模式
5. **apps/web/src/components/** - React 組件開發模式
6. **apps/web/src/lib/** - 前端工具函數和 tRPC Client 模式

#### 功能特定層 (5 個目錄)
7. **apps/web/src/app/api/** - Next.js API Routes 模式
8. **apps/web/src/i18n/** - next-intl 國際化配置模式
9. **apps/web/src/messages/** - 翻譯資源管理模式（CRITICAL）
10. **apps/web/src/hooks/** - React Custom Hooks 模式
11. **apps/web/e2e/** - Playwright E2E 測試模式

#### 配置與工具層 (4 個目錄)
12. **apps/web/src/components/ui/** - shadcn/ui 設計系統模式
13. **apps/web/src/components/layout/** - 佈局組件模式
14. **apps/web/src/components/providers/** - React Context Providers 模式
15. **packages/api/src/lib/** - API 工具層（EmailService）

### 3. 創建的文檔文件 (16 個)

| # | 文件路徑 | 內容概要 | 行數 |
|---|---------|---------|------|
| 1 | `.claude.md` | 根目錄概覽 - 專案結構、架構分層、核心技術棧 | 199 |
| 2 | `packages/api/src/routers/.claude.md` | tRPC Router CRUD 模式、權限控制、錯誤處理 | 273 |
| 3 | `packages/db/prisma/.claude.md` | Prisma Schema 模式、遷移工作流、關係模式 | 208 |
| 4 | `packages/auth/src/.claude.md` | NextAuth 配置、RBAC 實現、Session 管理 | 195 |
| 5 | `apps/web/src/app/[locale]/.claude.md` | CRUD 頁面模板、Server vs Client 組件 | 358 |
| 6 | `apps/web/src/components/.claude.md` | 組件分類、Props 模式、State 管理 | 272 |
| 7 | `apps/web/src/lib/.claude.md` | tRPC Client、React Query 配置、工具函數 | 157 |
| 8 | `apps/web/src/app/api/.claude.md` | API Routes、File Upload、Middleware | 127 |
| 9 | `apps/web/src/i18n/.claude.md` | next-intl 配置、Middleware、Server/Client 使用 | 126 |
| 10 | `apps/web/src/messages/.claude.md` | 翻譯資源、Key 命名規範、驗證工作流 | 147 |
| 11 | `apps/web/src/hooks/.claude.md` | Custom Hooks 模式、useTheme、useDebounce | 106 |
| 12 | `apps/web/e2e/.claude.md` | Playwright 測試、核心工作流、最佳實踐 | 186 |
| 13 | `apps/web/src/components/ui/.claude.md` | shadcn/ui 組件、變體模式、ARIA 標準 | 90 |
| 14 | `apps/web/src/components/layout/.claude.md` | DashboardLayout、Sidebar、TopBar 模式 | 86 |
| 15 | `apps/web/src/components/providers/.claude.md` | SessionProvider、Context 模式 | 75 |
| 16 | `packages/api/src/lib/.claude.md` | EmailService、SendGrid + Mailhog 配置 | 70 |

**總代碼行數**: 2,875 行 (平均每個文檔 ~180 行)

### 4. 文檔結構模式

每個 `.claude.md` 文件遵循統一結構:
...
1. **實用性導向**: 提供可直接使用的代碼模板和範例
2. **簡潔完整**: 不過度詳細,專注於核心信息
3. **視覺標記**: 使用 emoji 符號提高可讀性
4. **交叉引用**: 包含相關文件連結
5. **約定標記**: ⚠️ 符號標記重要規則
