...
- ✅ **完整 Schema 同步機制**
  ...
  - **核心文件**:
    - `packages/api/src/lib/schemaDefinition.ts` - 唯一真相來源 (31 個表格定義)
...
#### 12月16日
- ✅ **CHANGE-031: OM Summary Detail List 展開/收合一致性修復**
  - **問題**: 當 Header 下只有一個 OpCo 時，沒有展開/收合功能
  - **根因**: 代碼判斷 `header.opCoGroups.length > 1` 時才使用 Accordion
  - **修復**: 移除條件判斷，統一使用 Accordion 組件
  - **修改文件**: `apps/web/src/components/om-summary/OMSummaryDetailGrid.tsx`

- ✅ **CHANGE-029: Budget Category Statistics Admin 權限修復**
  - **問題**: 非 Admin 用戶仍可看到 Budget Category Statistics
  - **根因**: isAdmin 判斷邏輯不夠可靠
  - **修復**:
    - 增強 isAdmin 判斷：`role.name === 'Admin' OR role.id === 3`
    - 修復 TypeScript 錯誤：改用 `session?.user?.role?.id` 而非 `session?.user?.roleId`
    - 新增 Debug logging 方便問題排查
  - **修改文件**: `apps/web/src/app/[locale]/om-summary/page.tsx`

- ✅ **OM Summary Detail List 顏色調整**
  - Item 行背景色：`bg-pink-50` → `bg-white`
  - Subtotal 行背景色：`bg-orange-100/50` → `bg-gray-200` (#e5e7eb)
  - Subtotal 文字顏色：`text-orange-700` → `text-gray-700`
  - **修改文件**: `apps/web/src/components/om-summary/OMSummaryDetailGrid.tsx`

- ✅ **CHANGE-032: 用戶密碼設定與更改功能**
  - **需求**: Admin 可在用戶新增/編輯頁面設定或更改密碼
  - **密碼強度**: 12 字符以上，至少 6 個字符為大寫/數字/符號
  - **Phase 1: 後端 API**
    - 新增 `packages/api/src/lib/passwordValidation.ts` (驗證函數)
    - 新增 `user.setPassword` procedure (adminProcedure)
    - 新增 `user.hasPassword` procedure (publicProcedure)
    - 修改 `user.create` 支援可選 password 欄位
    - 安裝 bcryptjs 依賴 (salt rounds: 12)
  - **Phase 2: 前端組件**
    - 新增 `password-input.tsx` (顯示/隱藏切換)
    - 新增 `password-strength-indicator.tsx` (強度指示器)
    - 更新 `components/ui/index.ts` 導出
  - **Phase 3: 頁面整合**
    - 修改 `UserForm.tsx` 整合密碼設定功能
    - 建立模式: 可選密碼欄位
    - 編輯模式: 獨立更改密碼區塊
  - **i18n**: 新增 `users.password.*` 翻譯 (2579 → 2589 鍵)

- ✅ **FEAT-012: 統一載入特效系統**
  - **Phase 1: 核心組件開發**
    - 新增 `Spinner.tsx` (5 種尺寸, 4 種顏色)
    - 新增 `LoadingButton.tsx` (繼承 Button 所有功能)
    - 新增 `LoadingOverlay.tsx` (區域載入遮罩)
    - 新增 `GlobalProgress.tsx` (頂部導航進度條)
    - 新增 `loading/index.ts` (統一導出)
  - **Phase 2: 骨架屏組件** (已有 skeleton.tsx)
    - SkeletonTable, SkeletonCard, SkeletonText, SkeletonAvatar
  - **Phase 3: 全局整合**
    - 修改 `layout.tsx` 整合 GlobalProgress (Suspense 包裹)
    - 新增 `loading.*` i18n 翻譯 (2589 鍵)
  - **Phase 4: 頁面整合示範**
    - 修改 `VendorForm.tsx` 使用 LoadingButton
