...
- ✅ **CHANGE-033~035: UI 優化改進系列**
  - **CHANGE-033**: Login 頁面簡化
    - 移除「忘記密碼」連結
    - 移除「註冊」連結
    - 簡化頁面佈局
  - **CHANGE-034**: Projects 頁面預設值與過濾器
    - 新增搜尋欄位
    - 改進篩選功能佈局
    - 新增 i18n 翻譯 key
  - **CHANGE-035**: OM Expenses 頁面過濾器
    - 新增名稱搜尋功能（含 300ms 防抖）
    - API 新增 `search` 參數支援
    - 優化篩選器網格佈局（響應式設計）
    - 新增 i18n 翻譯 key
...
## 統計數據
- **新增文件**: 16 個 (10 rules + 3 FIX docs + 3 CHANGE docs)
- **修改文件**: 29 個
- **代碼行數**: +4,268 / -1,191
- **Commits**: 4 個
...
├── CHANGE-033-login-page-simplification.md
├── CHANGE-034-projects-page-defaults-and-filters.md
└── CHANGE-035-om-expenses-page-filters.md
```
...
- `apps/web/src/app/[locale]/login/page.tsx` (CHANGE-033)
- `apps/web/src/app/[locale]/projects/page.tsx` (CHANGE-034)
- `apps/web/src/app/[locale]/om-expenses/page.tsx` (CHANGE-035)
- `packages/api/src/routers/project.ts` (CHANGE-034)
- `packages/api/src/routers/omExpense.ts` (CHANGE-035)
- `apps/web/src/messages/zh-TW.json` (CHANGE-034, CHANGE-035 i18n)
- `apps/web/src/messages/en.json` (CHANGE-034, CHANGE-035 i18n)
