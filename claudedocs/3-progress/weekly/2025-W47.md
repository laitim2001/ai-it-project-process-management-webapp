...
### 本週提交記錄
```
[待提交] - docs(azure): 完成 Dev 環境首次部署 - 階段 0-2.11
e315b48 - fix(docker): 修復 Dockerfile Prisma 路徑 - 移除不必要的 .prisma 複製
3170caa - fix(nextauth): 簡化 webpack externals 配置 - 恢復 FIX-009 版本
189dd1e - fix(nextauth): 恢復 route.ts 解構賦值導出
c58f819 - fix(nextauth): 回滾錯誤的 JWT 模組路徑 - '@auth/core/jwt'
...
#### Azure Dev 環境首次部署 ✅
**狀態**: 完成 (100%)
**完成時間**: 2025-11-20 20:00 - 23:50
**執行階段**: 階段 0-2.11 全部完成

1. ✅ **Docker 建置修復（8 次嘗試）**
   - **問題**: NextAuth.js v5 配置錯誤導致建置失敗
   - **根本原因**: Post-MVP 版本與 FIX-009 成功版本不一致
   - **關鍵修復** (4 個 commits):
     - `c58f819`: 回滾錯誤的 JWT 模組路徑 (`'next-auth/jwt'` → `'@auth/core/jwt'`)
     - `189dd1e`: 恢復 route.ts 解構賦值導出 (`export const { GET, POST } = handlers;`)
     - `3170caa`: 簡化 webpack externals 配置（移除過度激進的配置）
     - `e315b48`: 修復 Dockerfile Prisma 路徑（移除不必要的 `.prisma` 複製）
   - **Build 嘗試記錄**:
     - Build 1-6 失敗: NextAuth SyntaxError (`Unexpected identifier 'as'`)
     - Build 7 失敗: Dockerfile 路徑錯誤 (`"/app/node_modules/.prisma": not found`)
     - Build 8 成功: 完整 67 個路由生成，映像大小 856 MB (ID: b871c1)

2. ✅ **Azure Container Registry 部署**
   - 登入 ACR: `az acr login --name acritpmdev` ✅
   - 推送映像: `docker push acritpmdev.azurecr.io/itpm-web:latest` (856 MB) ✅
   - 映像標籤: `latest` (基於 commit e315b48)

3. ✅ **PostgreSQL 防火牆規則配置**
   - 配置 20 個 App Service 出站 IP 地址
   - 允許範圍: 20.255.186.2 ~ 20.247.53.135
   - 方法: Azure CLI 批次創建

4. ✅ **App Service 重啟與遷移**
   - 重啟服務拉取新映像: `az webapp restart` ✅
   - 資料庫遷移成功 (3 個 migrations):
...
6. ✅ **Azure 資源清單**
   ...
   - Key Vault: `kv-itpm-dev` (12 個 secrets)

7. ✅ **技術亮點**
   - **系統性問題分析**: 深入對比 Git 歷史，找到 FIX-009 成功版本作為基準
   - **4 個精準修復**: 每個修復都針對具體問題，無副作用
   - **完整驗證**: 建置、部署、遷移、煙霧測試全部通過
   - **文檔完整性**: 所有過程記錄在 `AZURE-DEPLOY-PREP/06-deployment-execution-log.md`
...
- **代碼行數**: +2,700 / -50
- **文件變更**: 13 個新增/修改
- **Commits**: 16 次（含 Azure 部署修復）
- **工作時間**: ~12 小時（含 Azure 部署 4 小時）
- **完成度**: FEAT-001 **100%** ✅ | FEAT-002 Phase 1-2 **100%** ✅ | Azure Dev 部署 **100%** ✅
...
1. ✅ **問題發現與修復**
   - **問題1**: Login 頁面 Register 連結缺少 i18n 路由
     - 使用 `<a href="/register">` 導致跳轉至 `/register` 而非 `/zh-TW/register`
     - 修復：改用 `<Link>` 組件從 `@/i18n/routing`

   - **問題2**: Register 頁面僅有 mock 實現
     - 註冊表單顯示成功但未創建資料庫記錄
     - 無密碼加密
     - 無重複帳號檢查

2. ✅ **完整實現用戶註冊系統**
   - **新增檔案**: `apps/web/src/app/api/auth/register/route.ts` (229 行)
     - POST API endpoint 完整實現
...
3. ✅ **依賴安裝**
   - 安裝 bcrypt: `pnpm add bcrypt @types/bcrypt --filter=web`
   - 修正 monorepo workspace 安裝方式

4. ✅ **錯誤修復過程**
   - **錯誤1**: `ReferenceError: Link is not defined`
     - 原因：忘記 import Link
     - 修復：在 Line 61 添加 Link import

   - **錯誤2**: `Module not found: Can't resolve 'bcrypt'`
     - 原因：bcrypt 未安裝
...
7. ✅ **技術亮點**
   ...
   - **安全性**: bcrypt 10 rounds, 密碼不明文儲存
   - **i18n 路由**: 正確使用 next-intl Link 組件
   - **錯誤處理**: 區分驗證錯誤、重複帳號、系統錯誤
   - **用戶體驗**: 清晰的錯誤訊息和成功提示
...
7. 🚀 **Azure Dev 環境首次成功部署** - Docker 建置修復、ACR 推送、PostgreSQL 配置、資料庫遷移全部成功
8. 🔐 **用戶註冊系統完整實現** - 安全密碼加密、重複帳號檢查、完整 i18n 支援
